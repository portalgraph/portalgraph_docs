...
もし、PC内蔵の画面のすぐ上のWebカメラを使用し、一般的な16:9のモニターの場合、カメラタブで画面サイズ（インチ）を入力してOK（適用）をクリックするだけで設定は完了です。トラッカーアプリの自動起動や、顔に対するトラッカー位置（EyeCenter相当）はこのとき自動で設定されるので、以前のように個別に設定する必要はありません。
